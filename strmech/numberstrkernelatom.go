...
// # IMPORTANT
//
//	The instance of NumberStrKernel passed as input
//	parameter 'numStrKernel' will NOT BE MODIFIED by this
//	method.
//
// ----------------------------------------------------------------
//
...
//		*big.Float.
//
//		The data values contained in 'numStrKernel' WILL
...
//		'roundingType' is only applied in cases where the
//		current NumberStrKernel instance consists of a
//		floating point numeric value. In these cases,
//		if NumRoundType.NoRounding() is specified the
//		fractional digits will be truncated.
//
//		If in doubt as to a suitable rounding method,
//		'HalfAwayFromZero' is recommended.
//
...
//
//	Converts an instance of NumberStrKernel to a rational
//	number of type *big.Rat.
//
...
//		A pointer to an instance of NumberStrKernel. The
//		numeric value contained in this instance will be
//		converted and returned as a rational number of
//		type, *big.Rat.
//
//		The data values contained in 'numStrKernel' WILL
//		NOT BE MODIFIED by this method.
...
//		numeric	value represented by the NumberStrKernel
//		instance, 'numStrKernel', will be returned as a
//		rational number of type *big.Rat.
//
//	error
//
//		If this method completes successfully, this
...
//		prefixed or attached at the beginning of the error
//		message.
func (numStrKernelAtom *numberStrKernelAtom) convertKernelToBigRat(
	numStrKernel *NumberStrKernel,
	roundingType NumberRoundingType,
	roundToFactionalDigits int,
	errPrefDto *ePref.ErrPrefixDto) (
	...
	badZeros :=
		newNumStrKernel.integerDigits.GetCountLeadingZeros()

	if badZeros > 0 {

		err = newNumStrKernel.integerDigits.
	...
	badZeros =
		newNumStrKernel.fractionalDigits.GetCountTrailingZeros()

	if badZeros > 0 {

		err = newNumStrKernel.fractionalDigits.
			DeleteLeadingTrailingChars(
				badZeros,
				true,
				ePrefix.XCpy(
	...
	lenOfFracDigits := newNumStrKernel.fractionalDigits.GetRuneArrayLength()

	if lenOfFracDigits > 0 {
	...
	exponent := big.NewInt(int64(lenOfFracDigits))
