...
func (stdLine *TextLineSpecStandardLine) Equal(
	...
	if stdLine.numOfStdLines !=
		incomingStdLine.numOfStdLines {

		return false
	}

	lenCurrTextFields := len(stdLine.textFields)

	if lenCurrTextFields !=
		len(incomingStdLine.textFields) {
		return false
	}

	if lenCurrTextFields > 0 {
		for i := 0; i < lenCurrTextFields; i++ {

			if !stdLine.textFields[i].EqualITextField(
				incomingStdLine.textFields[i]) {
				return false
			}
